...
def getScore(state):
    ...
    state.orderedMoves = state.valid_actions()  # Assuming valid_actions gives the ordered moves
    for move in state.orderedMoves:
        ...
        if state.board[state.current_player(), move[0], move[1]] == 1:
            ...
            for vector in boundaryList:
                ...
                # Extending heads
                while not outOfRange(head) and state.board[state.current_player(), head[0], head[1]] == 1:
                    curLen += 1
                    head = (head[0] + vector[0], head[1] + vector[1])
                headBlock = outOfRange(head) or state.board[EMPTY, head[0], head[1]] == 0

                # Extending tails
                while not outOfRange(tail) and state.board[state.current_player(), tail[0], tail[1]] == 1:
                    curLen += 1
                    tail = (tail[0] - vector[0], tail[1] - vector[1])
                tailBlock = outOfRange(tail) or state.board[EMPTY, tail[0], tail[1]] == 0
